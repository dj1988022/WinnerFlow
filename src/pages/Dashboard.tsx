...
export default function Dashboard() {
  ...
  const handleRunAnalysis = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const { error } = await supabase.from('user_monitor_tasks').upsert({ 
      ...
      if (error) {
        ...
        alert("Success! Tracking: " + currentQuery);
        setSearchQuery('');
      }
    ...
    <div className="space-y-6 text-white p-4">
      <div className="flex flex-col md:flex-row justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm">Real-time market intelligence.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="bg-emerald-600 hover:bg-emerald-500 px-6 py-2 rounded-lg text-sm font-bold disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 h-[350px]">
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 h-[350px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={categoryData} layout="vertical">
              <YAxis dataKey="name" type="category" width={60} stroke="#a1a1aa" />
